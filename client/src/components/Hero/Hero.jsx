import { DotLottiePlayer } from "@dotlottie/react-player";
import W from "./W";
import WOWlogo from "../../assets/images/wow_logo_l.png";

function Hero() {
  return (
    <div className="flex flex-col relative items-center justify-center h-screen w-full overflow-hidden">
      <DotLottiePlayer
        src={"./wave.lottie"}
        autoplay
        loop
        speed={0.3}
        className=" absolute md:w-[150%] md:h-[200%] -top-[85%] -left-[80%] rotate-[300deg]"
      />
      <DotLottiePlayer
        src={"./wave.lottie"}
        autoplay
        loop
        speed={0.3}
        className=" absolute md:w-[150%] md:h-[200%] -bottom-[75%] -right-[80%] rotate-[15deg]"
      />
      <main className="z-10 px-10 flex flex-col items-center justify-center gap-10">
        <div className="flex items-center justify-center">
          <img
            src={WOWlogo}
            alt="WOW Logo"
            className="w-[80%] sm:w-[65%] md:w-[50%] object-contain"
          />
        </div>
        <div className="flex flex-col items-center justify-center gap-5">
          <div className="text-4xl text-center leading-normal">
            <span className=" text-[#FBBC04] ">Google </span>
            <span className=" text-[#EA4335]">Developer </span>
            <span className=" text-[#0F9654]">Student </span>
            <span className=" text-[#4A80EB]">Clubs</span>
          </div>
          <div className=" text-4xl text-[#FFFFFF]">Delhi NCR</div>
        </div>
      </main>
      <W />
    </div>
  );
}

export default Hero;
